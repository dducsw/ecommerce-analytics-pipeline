...
import pandas as pd
from sqlalchemy import create_engine, text, MetaData, Table, Column, Integer, BigInteger, String, Float, DateTime, TIMESTAMP
...
def ensure_database_exists(host: str, port: int, user: str, password: str, db_name: str):
    """Ensure the target database exists, create if not"""
    # Connect to default 'postgres' database to check/create target database
    default_conn_string = f"postgresql+psycopg2://{user}:{password}@{host}:{port}/postgres"
    default_engine = create_engine(default_conn_string, isolation_level="AUTOCOMMIT", echo=False)
...
def get_db_engine(host: str, port: int, user: str, password: str, db_name: str) -> Engine:
    ...
    try:
        ...
        for i in range(0, total_rows, chunksize):
            ...
            if i % progress_report_interval == 0 or loaded == total_rows:
                ...
                logger.info(f"  Progress: {loaded:,}/{total_rows:,} rows ({percentage}%)")
...
def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info(f"Successfully loaded: {len(successful_loads)}/{len(tables_to_load)} tables")
        if failed_loads:
        ...
        logger.info(f"\nNext steps:")
        logger.info(f"1. Verify data: psql -U {args.user} -d {args.db_name} -c 'SELECT COUNT(*) FROM {args.schema}.products;'")
